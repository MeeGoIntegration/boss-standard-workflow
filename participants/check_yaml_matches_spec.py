    ...
    def handle_wi(self, wid):
        ...
        for action in wid.fields.ev.actions:
            if action["type"] != "submit":
                # Not an SR, skip
                continue
            result = result and self.__handle_action(wid, action)
        if not result:
            wid.fields.status = "FAILED"
        wid.result = result

    def __handle_action(self, wid, action):
        ...
        with Lab() as lab:
            ...
            if not (spec and
                ...
                return True
            if not yaml:
                wid.fields.msg.append("SPEC file generated with spectacle, "
            ...
            rcode = specify.wait()
            if rcode != 0:
                wid.fields.msg.append("Running specify failed:\n%s" %
                        specify.stdout.read())
                return False
            # Get the diff
